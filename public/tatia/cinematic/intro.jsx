/* ===========================================================
   intro.jsx — CinematicIntro (timed sequence) + AnimatedTitle
   =========================================================== */
...
/* ---- AnimatedTitle: the crisp materialising wordmark ---- */
...
/* ---- CinematicIntro: full timed opening ---- */
function CinematicIntro({ engine, onReady }) {
  const [phase, setPhase] = useState(0);   // 0 dark, 1 line, 2 building, 3 title, 4 invite, 5 cue
  ...
  useEffect(() => {
    ...
    function finish() {
      ...
      // hand off: set particles to final title shape, hidden, ready to shatter on scroll
      if (engine) {
    ...
    // 1) gold flourish draws
    T(() => { setDrawLine(true); setPhase(1); }, 350);

    // 2) particles gather → "Tatia's"
    T(() => {
      setPhase(2);
    ...
    // 4) particles crystallise → fade engine down while crisp HTML title materialises
    T(() => {
      setPhase(3);
  ...
  return (
    <div className="relative w-full" style={{ minHeight: '100svh' }}>
      <div className="absolute inset-0 flex flex-col items-center justify-center px-7"
        ...
        {/* crisp title (materialises after particle build) */}
        <div className="relative z-10">
          <AnimatedTitle show={phase >= 3} />
        </div>

        {/* georgian invitation line */}
        <div className="relative z-10 mt-9 max-w-[20rem] text-center"
          ...
          <p className="font-geo text-[15px] leading-[1.85] text-pearl/80" style={{ fontWeight: 300 }}>
...
Object.assign(window, { CinematicIntro, AnimatedTitle, tween, easeInOut, easeOut });
