"""Generate print-style preview PDF for GBMN peer-review report.

Usage:
  python3 tools/peer_review_preview.py
  python3 tools/peer_review_preview.py --output previews/custom.pdf --logo public/gbmn-logo.png
"""
...
try:
    ...
    print(
        ...
        "Install locally with: pip install reportlab\n"
        "Then rerun: python3 tools/peer_review_preview.py",
        file=sys.stderr,
    )
...
class ColorHR(Flowable):
    ...
    def draw(self):
        width = (PAGE_W - MARGIN_L - MARGIN_R) * self.width_pct
        self.canv.setStrokeColor(self.color)
        self.canv.setLineWidth(self.thickness)
        self.canv.line(0, self.thickness / 2, width, self.thickness / 2)
...
def header_footer(canvas_obj, doc, logo_path: str):
...
def section_header(title, styles):
    table = Table([[Paragraph(title, styles['section_heading'])]], colWidths=[PAGE_W - MARGIN_L - MARGIN_R])
...
def build_pdf(output_path: str, logo_path: str):
    ...
    story = [
        Spacer(1, 10),
        Paragraph('PEER REVIEW REPORT', styles['title']),
        ColorHR(TEAL, thickness=2.5),
        Spacer(1, 6),
        Paragraph('Dermatoscopic Vascular Patterns and Invasive Tumor Depth as Predictors of Aggressive Behavior in Basal Cell Carcinoma', styles['subtitle']),
        Spacer(1, 10),
        section_header('1. SUMMARY', styles),
        Spacer(1, 7),
        Paragraph('This is a generated preview sample from your provided ReportLab code.', styles['body']),
        Paragraph('Use this PDF to visually verify print layout, header spacing, and typography.', styles['body']),
    ]
    doc.build(
        story,
        onFirstPage=lambda c, d: header_footer(c, d, logo_path),
        onLaterPages=lambda c, d: header_footer(c, d, logo_path),
    )


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--output', default='previews/GBMN_Peer_Review_Report_preview.pdf')
    parser.add_argument('--logo', default='public/gbmn-logo.png')
    args = parser.parse_args()

    os.makedirs(os.path.dirname(args.output) or '.', exist_ok=True)
    build_pdf(args.output, args.logo)
    print(args.output)
